...
# Configuration
REPO_OWNER = "Lolliedieb"
REPO_NAME = "lolMiner-releases"
GITHUB_API_URL = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/releases/latest"

def get_project_root():
    return Path(__file__).parent.parent

def install_lolminer():
    print("--- Starting lolMiner Installation ---")
    
    project_root = get_project_root()
    install_dir = project_root / "miners" / "lolminer"
    ...
    try:
        ...
        # lolMiner usually has a wrapper dir like "1.84_Win64"
    ...
    # 7. Check for executable
    exe_path = install_dir / "lolMiner.exe"
    if exe_path.exists():
        print(f"SUCCESS: lolMiner installed at {exe_path}")
        return True
    else:
        print(f"Error: lolMiner.exe not found in {install_dir} after extraction.")
        return False

if __name__ == "__main__":
    success = install_lolminer()
    if not success:
        sys.exit(1)
